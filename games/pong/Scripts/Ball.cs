...
public class Ball : MonoBehaviour {
	...
	void OnCollisionEnter2D(Collision2D col){
		...
		if ((col.gameObject.name == "Player") || (col.gameObject.name == "CPU")) {
			handlePaddleHit (col);
		}
 
		if ((col.gameObject.name == "South") || (col.gameObject.name == "North")) {
 
			// Call for SoundManager to play the wall sound
			SoundManager.Instance.PlayOneShot (SoundManager.Instance.wallBloop);
		}
 
		if ((col.gameObject.name == "West") || (col.gameObject.name == "East")) {
			...
			if (col.gameObject.name == "West") {
				...
				increaseTextUIScore ("PlayerScoreUI");
					
			} else if (col.gameObject.name == "East") {
 
				increaseTextUIScore ("CPUScoreUI");
